...
impl Crates {
  ...
  pub fn search(&mut self, query: &str) -> Result<Vec<Crate>> {
    let body = try!(self.get(format!("/crates?q={}", query), Auth::Unauthorized));
  ...
  pub fn named(&mut self, name: &str) -> Result<String> { // can't decode this automagically because it contains the key "crate"!
    self.get(format!("/crates/{}", name), Auth::Unauthorized)
  }

  fn get(&mut self, path: String, auth: Auth) -> Result<String> {
    self.req(path, None, Method::Get, auth)
  }

  fn req(&mut self, path: String, body: Option<&[u8]>, method: Method, auth: Auth) -> Result<String> {
